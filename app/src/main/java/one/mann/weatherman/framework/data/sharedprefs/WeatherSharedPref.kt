...
internal class WeatherSharedPref(private val context: Context) {
    ...
    val updateAll: Boolean
        get() = mainPref.getBoolean(UPDATE_ALL, false)

    fun cityPref(name: String): SharedPreferences = context.getSharedPreferences(name, Context.MODE_PRIVATE)

    suspend fun getWeatherData(key: String, pref: SharedPreferences): String? =
            withContext(Dispatchers.IO) { pref.getString(key, "") }

    fun getSunPosition(pref: SharedPreferences): Float = pref.getFloat(SUN_POSITION, 0f)
